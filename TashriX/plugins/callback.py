...
from TashriX import YouTube, app
from TashriX.core.call import Tashri
from TashriX.misc import SUDOERS, db
from TashriX.utils import bot_sys_stats
from TashriX.utils.database import (
    get_active_chats,
    get_lang,
    is_active_chat,
    is_music_playing,
...
from TashriX.utils.formatters import seconds_to_min
from TashriX.utils.inline import (
    stream_markup,
    stream_markup_timer,
    telegram_markup,
...
from TashriX.utils.stream.autoclear import auto_clean
from TashriX.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Tashri.pause_stream(chat_id)
        ...
        await Tashri.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Tashri.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Tashri.skip_stream(chat_id, link, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await Tashri.skip_stream(chat_id, queued, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
